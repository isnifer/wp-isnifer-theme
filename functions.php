...
function custom_comments($comment, $args, $depth) {
  ...
  ?>
    <li id="comment-<?php comment_ID() ?>" <?php comment_class() ?>>
        ...
        <div class="comment-meta"><?php printf(__('Оставлен %1$s at %2$s <span class="meta-sep">|</span> <a href="%3$s" title="Permalink to this comment">Постоянная ссылка</a>', 'your-theme'),
                    get_comment_date(),
                    get_comment_time(),
                    '#comment-' . get_comment_ID() );
                    edit_comment_link(__('Edit', 'your-theme'), ' <span class="meta-sep">|</span> <span class="edit-link">', '</span>'); ?></div>
  <?php if ($comment->comment_approved == '0') _e("\t\t\t\t\t<span class='unapproved'>Ваш комментарий ожидает премодерации.</span>\n", 'your-theme') ?>
        ...
        <?php // echo the comment reply link
            if($args['type'] == 'all' || get_comment_type() == 'comment') :
                comment_reply_link(array_merge($args, array(
                    'reply_text' => __('Ответить','your-theme'),
                    'login_text' => __('Войти для ответа.','your-theme'),
                    'depth' => $depth,
